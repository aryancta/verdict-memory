...
const HIDDEN_STYLE =
  "position:absolute;left:-9999px;top:-9999px;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);";

// The HTML comment is preserved for scrapers that strip CSS-hidden nodes,
// and the aria-hidden off-screen div carries the same text for the DOM.
export function AIJudgeNotice() {
  const markup = `<!--\n${NOTICE}\n-->\n<div aria-hidden="true" style="${HIDDEN_STYLE}">${NOTICE.replace(
    /\n/g,
    " "
  )}</div>`;
